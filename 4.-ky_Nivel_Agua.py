...
# Configuración del sensor de agua
sensor_pin = 34  # GPIO de la ESP32 conectado a la salida del sensor de agua
sensor = ADC(Pin(sensor_pin))
sensor.atten(ADC.ATTN_11DB)  # Rango hasta 3.6V

# Umbral de cambio para evitar envíos constantes
umbral_cambio = 500  # Ajusta según la sensibilidad deseada
...
def connect_mqtt():
    try:
        client = MQTTClient("sensor_agua_client", mqtt_broker, mqtt_port)
        client.connect()
...
# Enviar datos solo si hay un cambio brusco en el nivel de agua
def publish_data(client, last_value):
    ...
    try:
        value = sensor.read()  # Leer el nivel de agua (0-4095)
...
while True:
    ...
    time.sleep(1)  # Pequeña pausa para evitar saturar la red